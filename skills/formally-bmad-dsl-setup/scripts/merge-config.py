...
def variable_definitions(module_yaml: dict) -> dict[str, dict]:
    """Return variable definitions keyed by variable key.

    Supports the BMad module.yaml shape where variables live under a top-level
    `variables` list.
    """
    definitions: dict[str, dict] = {}
    for entry in module_yaml.get("variables", []):
...
def load_legacy_values(
    ...
    legacy_module: dict = {}
    files_found: list = []
    definitions = variable_definitions(module_yaml)
...
def apply_result_templates(
    ...
    for key, value in module_answers.items():
        ...
        if (
            ...
            transformed[key] = template.replace("{answer}", str(value)).replace("{value}", str(value))
            if verbose:
                print(
