...
# ///
"""Initialize Formally BMAD DSL project state and detect reasoning tools."""
...
def ensure_structure(module_root: Path, canonical_path: Path, status_title: str, index_title: str) -> list[str]:
    folders = [
        ...
        module_root / "artifacts" / "dsl-brainstorming",
        module_root / "artifacts" / "dsl-prd",
    ...
    if not index.exists():
        index.write_text(f"# {index_title}\n\nInitialized by setup.\n", encoding="utf-8")
    return created
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Initialize Formally BMAD DSL state and detect automated reasoning tools.")
    parser.add_argument("--project-root", required=True, help="Project root directory.")
    parser.add_argument("--module-root", required=True, help="Formally BMAD DSL module state directory.")
    ...
    parser.add_argument("--module-label", default="Formally BMAD DSL", help="Human-readable module label for reports.")
    parser.add_argument("--status-title", default="Formally BMAD DSL Canonical Model Status", help="Title for canonical status file.")
    parser.add_argument("--index-title", default="Formally BMAD DSL Index", help="Title for generated index file.")
    ...
    created = ensure_structure(module_root, canonical_path, args.status_title, args.index_title)
    ...
    report_path = write_report(module_root, result, args.module_label)
