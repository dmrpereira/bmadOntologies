...
class ContractStubsWorkspaceTests(unittest.TestCase):
    def test_slugify_defaults_to_contract_stubs(self):
        self.assertEqual(contract_stubs_workspace.slugify("   "), "contract-stubs")
    ...
    def test_create_workspace_writes_stub_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = contract_stubs_workspace.create_workspace(module_root, "python parser api")
            workspace = Path(result["workspace"])

            self.assertTrue((workspace / "stubs.md").is_file())
            self.assertTrue((workspace / "review-checklist.md").is_file())
